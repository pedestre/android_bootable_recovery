...
char** gather_files(const char* directory, const char* fileExtensionOrDirectory, int* numFiles)
{
    ...
    *numFiles = 0;
    ...
    if(closedir(dir) < 0) {
        LOGE("Fallo al cerrar directorio.");
    }
...
void show_choose_zip_menu(const char *mount_point)
{
    ...
    static char* headers[] = {  "elegir un zip a aplicar",
                                "",
                                NULL
    };
...
void show_nandroid_restore_menu(const char* path)
{
    ...
    static char* headers[] = {  "Elegir imagen para Restaurar",
                                "",
                                NULL
    };
...
void show_nandroid_delete_menu(const char* path)
{
    ...
    static char* headers[] = {  "Elegir una imagen para Borrar",
                                "",
                                NULL
    };
...
int control_usb_storage_set_lun(Volume* vol, bool enable, const char *lun_file) {
    ...
    if ((fd = open(lun_file, O_WRONLY)) < 0) {
        LOGW("Imposible abrir ums lunfile %s (%s)\n", lun_file, strerror(errno));
        return -1;
    }
    ...
    if ((write(fd, vol_device, strlen(vol_device)) < 0) &&
       (!enable || !vol->device2 || (write(fd, vol->device2, strlen(vol->device2)) < 0))) {
        LOGW("Imposible escribir a ums lunfile %s (%s)\n", lun_file, strerror(errno));
        close(fd);
        return -1;
    } else {
...
void show_mount_usb_storage_menu()
{
    ...
    static char* headers[] = {  "Unidad de Almacenamiento Masivo USB",
                                "Abandonando este menu se ",
                                "desmontara su SD card de su PC",
                                "",
                                NULL
    };
...
int format_device(const char *device, const char *path, const char *fs_type) {
    ...
    if (strcmp(fs_type, "ramdisk") == 0) {
        ...
        LOGE("no es posible formatear el volumen \"%s\"", path);
        return -1;
    }

    if (strcmp(fs_type, "rfs") == 0) {
        if (ensure_path_unmounted(path) != 0) {
            LOGE("format_volume fallo al desmontar \"%s\"\n", v->mount_point);
            return -1;
        }
        if (0 != format_rfs_device(device, path)) {
            LOGE("format_volume: format_rfs_device fallo en %s\n", device);
            return -1;
        }
        return 0;
    }
    ...
    if (strcmp(fs_type, "yaffs2") == 0 || strcmp(fs_type, "mtd") == 0) {
        ...
        if (partition == NULL) {
            LOGE("format_volume: no existe la particion MTD \"%s\"\n", device);
            return -1;
        }

        MtdWriteContext *write = mtd_write_partition(partition);
        if (write == NULL) {
            LOGW("format_volume: no se puede abrir MTD \"%s\"\n", device);
            return -1;
        } else if (mtd_erase_blocks(write, -1) == (off_t) -1) {
            LOGW("format_volume: no se puede borrar MTD \"%s\"\n", device);
            mtd_write_close(write);
            return -1;
        } else if (mtd_write_close(write)) {
            LOGW("format_volume: no se puede cerrar MTD \"%s\"\n",device);
            return -1;
        }
        return 0;
    }
    ...
    if (strcmp(fs_type, "ext4") == 0) {
        ...
        if (result != 0) {
            LOGE("format_volume: make_extf4fs fallo en %s\n", device);
            return -1;
        }
        return 0;
    }
...
{
    LOGI("Formatting unidad desconocida.\n");
    ...
    if (NULL != fs_type) {
        if (strcmp("ext3", fs_type) == 0) {
            LOGI("Formateando la unidad ext3.\n");
        ...
        if (strcmp("ext2", fs_type) == 0) {
            LOGI("Formateando la unidad ext2.\n");
            if (0 != ensure_path_unmounted(path)) {
                LOGE("Error mientras se desmontaba %s.\n", path);
                return -12;
            }
...
void show_nandroid_advanced_restore_menu(const char* path)
{
    ...
    static char* advancedheaders[] = {  "Elegir imagen para Restaurar",
                                "",
                                "Elegir imagen para Restaurar",
                                "En el siguiente menu",
                                "tendra mas opciones.",
                                "",
                                NULL
    };
    ...
    static char* headers[] = {  "Restauracion Avanzada",
                                "",
                                NULL
    };
...
{
    static char* headers[] = {  "Copia Seguridad-Restauracion",
                                "",
                                NULL
    };

    char* list[] = { "Hacer Copia de Seguridad",
                            "Restaurar",
                            "Borrar copia",
                            "Restauracion Avanzada",
                            "Liberar espacio de copias no usadas",
                            "Elegir formato de Copia de Seguridad",
                            NULL,
                            NULL,
                            NULL,
                            NULL,
...
int can_partition(const char* volume) {
    Volume *vol = volume_for_path(volume);
    if (vol == NULL) {
        LOGI("No se puede particionar el volumen desconocido: %s\n", volume);
        return 0;
    }
    ...
    if (strcmp(vol->fs_type, "vfat") != 0) {
        LOGI("No se puede particionar non-vfat: %s\n", vol->fs_type);
        return 0;
    }
...
void show_advanced_menu()
{
    ...
    static char* list[] = { "Reiniciar en modo Recovery",
                            "Borrar dalvik cache (Wipe dalvik)",
                            "Borrar estadisticas de bateria",
                           // "Guardar log de error",
                           // "key test",
                           // "Mostrar log",
                            "Arreglar Permisos (Fix permissions)",
                            NULL,
                            NULL,
                            NULL,
			    NULL,
    ...
    for (;;)
    {
        ...
        switch (chosen_item)
        {
            ...
            case 4:
            {
                ui_print("Extrayendo Key Codes.\n");
                ui_print("Volver para finalizar depuracion.\n");
                int key;
                int action;
                do
                {
...
void write_fstab_root(char *path, FILE *file)
{
    ...
    if (vol == NULL) {
        LOGW("Imposible conseguir la informacion de recovery.fstab para %s durante la generacion del fstab!\n", path);
        return;
    }
...
void create_fstab()
{
    ...
    if (file == NULL) {
        LOGW("No es posible crear /etc/fstab!\n");
        return;
    }
    ...
    fclose(file);
    LOGI("Completado el outputting fstab.\n");
}
...
int is_path_mounted(const char* path) {
    ...
    if (result < 0) {
        LOGE("Fallo al buscar volumenes montados\n");
        return 0;
    }
