...
int run_and_remove_extendedcommand()
{
    ...
    int i = 0;
    for (i = 20; i > 0; i--) {
        ui_print("Esperando a que la SD Card (%ds) este montada\n", i);
        if (ensure_path_mounted("/sdcard") == 0) {
            ui_print("SD Card montada...\n");
            break;
        }
        sleep(1);
    }
    ...
    if (i == 0) {
        ui_print("Tiempo de espera para la SD Card agotado... se continua igualmente.");
    }

    ui_print("Verificando el marcador de la SD Card...\n");
    struct stat st;
    if (stat("/sdcard/clockworkmod/.salted_hash", &st) != 0) {
        ui_print("No se encuentra el marcador de la SD Card...\n");
        if (volume_for_path("/emmc") != NULL) {
            ui_print("Comprobando marcador de la SD Interna...\n");
            ensure_path_unmounted("/sdcard");
            if (ensure_path_mounted_at_mount_point("/emmc", "/sdcard") != 0) {
                ui_print("Marcador de la SD Interna no encontrado... se continua igualmente.\n");
    ...
    if (0 != (ret = before_run_script(tmp))) {
        ui_print("Error procesando script de ROM Manager.\n");
        return ret;
    }
#endif
