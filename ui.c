...
UIParameters ui_parameters = {
    ...
    20,      // fps
    7,       // installation icon frames (0 == static image) default 7
    13, 190, // installation icon overlay offset Default 13, 190,
};
...
static const struct { gr_surface* surface; const char *name; } BITMAPS[] = {
    { &gBackgroundIcon[BACKGROUND_ICON_INSTALLING], "icon_installing" },  //icon_installing
...
{
    gPagesIdentical = 0;
//NUEVO PROBAR (dos primeras lineas comentadas)
    //gr_color(0, 0, 0, 255);
...
//Fin de nuevo
    //if (icon) {
        ...
        int iconY = 5*((gr_fb_height() - iconHeight) / 6);
...
//NUEVO, comentadas las dos siguientes lineas
        gr_color(0, 0, 0, 160);
        gr_fill(0, 0, gr_fb_width(), gr_fb_height());
	gr_surface surface = gVirtualKeys;
//NUEVO cambiada siguiente linea
...
//        return 0;
   // Touch to select cambio de linea
...
//        boardEnableKeyRepeat = 0;
	...
	// if (touch_y < (gr_fb_height() - gr_get_height(surface))) {
	   ...
	   if(touch_y < (gr_fb_height() - gr_get_height(surface))) {
		if(diff_x > X_SWIPE_THRESH) {
                ...
                slide_right = 1;
                reset_gestures();
	   //touch } else if(diff_x < ((gr_fb_width() / 4) * -1)) {
    ...
    //touch } else if (ev.type == 3 && ev.code == 54) {
   ...
   //touch if (touch_y < (gr_fb_height() - gr_get_height(surface))) {
   //touch         if (diff_y > 25) {
     if(touch_y < (gr_fb_height() - gr_get_height(surface))) {
